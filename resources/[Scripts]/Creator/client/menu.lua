...
identity = RageUI.CreateSubMenu(mainMenu, Locales[Config.Locale]['identity_title'], Locales[Config.Locale]['identity_subtitle'])
...
RageUI.CreateWhile(1.0, mainMenu, nil, function()
	...
	RageUI.IsVisible(mainMenu, true, true, true, function()
		...
		-- Identité
		RageUI.Button(Locales[Config.Locale]['identity_item'], Locales[Config.Locale]['identity_item_desc'],{}, true, function(Hovered, Active, Selected)
			if Selected then
			end
		end, identity)
		...
		RageUI.List(Locales[Config.Locale]['clothe_item'], ClotheList, actionClothe, Locales[Config.Locale]['clothe_item_desc'], {}, true, function(Hovered, Active, Selected, Index)
			...
			actionClothe = Index
			LoadAnim("mp_clothing@female@shoes")
            TaskPlayAnim(GetPlayerPed(-1), "mp_clothing@female@shoes", "try_shoes_positive_a", 1.0, 1.0, -1, 0, 1, 0, 0, 0)
	...
	-- Identité Menu
	RageUI.IsVisible(identity, true, true, true, function()
		RageUI.Button("Prénom", "Inscrivez votre ~b~Prénom~s~", {RightLabel = nameidentity}, true, function(Hovered, Active, Selected)
			...
			if (Selected) then 
				...
				if tostring(nameInput) == nil then
					...
					TriggerServerEvent("val_i:UpdateName", tostring(nameInput))
		...
		RageUI.Button("Nom", "Inscrivez votre ~b~Nom~s~", {RightLabel = prenomidentity}, true, function(Hovered, Active, Selected)
			if (Selected) then 
				...
				if tostring(prenomInput) == nil then
					...
					TriggerServerEvent("val_i:UpdatePrenom", tostring(prenomInput))
		...
		RageUI.Button("Taille", "Inscrivez votre ~b~Taille ~s~(cm) dans le champ indiqué", {RightLabel = tailleidentity}, true, function(Hovered, Active, Selected)
			if (Selected) then 
				...
				if tostring(tailleInput) == nil then
					...
					tailleidentity = (tostring(tailleInput))
	
					TriggerServerEvent("val_i:UpdateTaille", tostring(tailleInput))
		...
		RageUI.Button("Sexualité", "Inscrivez ~b~H~s~ ou ~p~F~s~ dans le champ indiqué", {RightLabel = sexidentity}, true, function(Hovered, Active, Selected)
			if (Selected) then 
				...
				if tostring(sexInput) == nil then
					...
					sexidentity = (tostring(sexInput))
	
					TriggerServerEvent("val_i:Updatesex", tostring(sexInput))
		...
		RageUI.Button("Date de Naissance", "Inscrivez votre ~b~Date de Naissance", {RightLabel = dateidentity}, true, function(Hovered, Active, Selected)
			if (Selected) then 
				...
				if tostring(dateInput) == nil then
					...
					dateidentity = (tostring(dateInput))
	
					TriggerServerEvent("val_i:Updatedate", tostring(dateInput))
